...
import { WorkoutPlan, GenerateWorkoutRequest, Exercise } from '../../../shared/types';
...
export default function WorkoutsScreen() {
  ...
  }, [showExercisePicker, pickerSearch, pickerMuscleFilter]);
  ...
  const loadPickerExercises = async (query: string) => {
    try {
      ...
      setPickerMuscleGroups(allMuscles);
      const filtered = data.filter((e: any) => {
        const matchesSearch = !query ||
          e.name.toLowerCase().includes(query.toLowerCase()) ||
          e.primaryMuscles.some((m: string) => m.toLowerCase().includes(query.toLowerCase()));
        const matchesMuscle = pickerMuscleFilter === 'all' ||
          e.primaryMuscles.some((m: string) => m.toLowerCase() === pickerMuscleFilter.toLowerCase());
  ...
  return (
    <View style={styles.container}>
      ...
      <Modal visible={showExercisePicker} animationType="slide" presentationStyle="pageSheet" onRequestClose={() => { setShowExercisePicker(false); setPickerMuscleFilter('all'); setTimeout(() => setShowPlanBuilder(true), 350); }}>
        <View style={styles.modalContainer}>
          <View style={styles.modalHeader}>
            <Text style={styles.modalTitle}>Add Exercise</Text>
            <AnimatedButton variant="ghost" size="small" onPress={() => { setShowExercisePicker(false); setPickerMuscleFilter('all'); setTimeout(() => setShowPlanBuilder(true), 350); }} title="Back" textStyle={{ color: COLORS.primary }} />
          </View>
          <View style={styles.searchContainer}>
            <Ionicons name="search" size={18} color={COLORS.textSecondary} />
            <TextInput
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.muscleFilterRow} contentContainerStyle={styles.muscleFilterContent}>
            <TouchableOpacity
              style={[styles.muscleChip, pickerMuscleFilter === 'all' && styles.muscleChipActive]}
              onPress={() => setPickerMuscleFilter('all')}
            >
              <Text style={[styles.muscleChipText, pickerMuscleFilter === 'all' && styles.muscleChipTextActive]}>All</Text>
            </TouchableOpacity>
            {pickerMuscleGroups.map(muscle => (
              <TouchableOpacity
                key={muscle}
                style={[styles.muscleChip, pickerMuscleFilter === muscle && styles.muscleChipActive]}
                onPress={() => setPickerMuscleFilter(pickerMuscleFilter === muscle ? 'all' : muscle)}
              >
                <Text style={[styles.muscleChipText, pickerMuscleFilter === muscle && styles.muscleChipTextActive]}>{muscle}</Text>
              </TouchableOpacity>
            ))}
          </ScrollView>
          <Animated.ScrollView style={styles.exerciseList}>
            ...
            {customExercises.filter(e => {
              const matchesSearch = !pickerSearch || e.name.toLowerCase().includes(pickerSearch.toLowerCase());
              const matchesMuscle = pickerMuscleFilter === 'all' || e.muscleGroup?.toLowerCase() === pickerMuscleFilter.toLowerCase();
              return matchesSearch && matchesMuscle;
            }).map((exercise, index) => (
              <AnimatedListItem key={`custom-${exercise.id}`} index={index} enterFrom="right">
                <AnimatedCard
                  onPress={() => addExerciseToDay(editingDayIndex, { id: null, name: exercise.name, primaryMuscles: [exercise.muscleGroup] })}
