...
export const NutritionProvider: React.FC<NutritionProviderProps> = ({ children }) => {
  ...
  const refreshToday = useCallback(async () => {
    try {
      const summary = await nutritionApi.getTodayLogs();
  ...
  const refreshTargets = useCallback(async () => {
    try {
      const plan = await nutritionApi.calculateNutrition();
      setTargets(plan);
    } catch (err) {
  ...
  const refreshStreak = useCallback(async () => {
    try {
      const streakData = await nutritionApi.getStreak();
  ...
  const refreshWeightTrend = useCallback(async () => {
    try {
      const trend = await nutritionApi.getWeightTrend();
  ...
  const refreshRecentFoods = useCallback(async () => {
    try {
      const recent = await nutritionApi.getRecentFoods();
  ...
  const logFood = useCallback(async (
    ...
    try {
      const result = await nutritionApi.logFoodWithStreak({ foodId, servings, mealType });
  ...
  const updateLog = useCallback(async (
    ...
    try {
      await nutritionApi.updateFoodLog(logId, { servings, mealType });
      await refreshToday();
    } catch (err) {
  ...
  const logWeight = useCallback(async (weight: number, note?: string) => {
    try {
      await nutritionApi.logWeight(weight, note);
