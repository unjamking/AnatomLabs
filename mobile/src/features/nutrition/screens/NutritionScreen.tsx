...
import { useWorkoutTracking } from '../../../features/workouts/context/WorkoutTrackingContext';
import {
  AnimatedCard,
  AnimatedButton,
  AnimatedListItem,
...
type TabType = 'diary' | 'nutrients' | 'trends' | 'goals' | 'diet';
...
export default function NutritionScreen() {
  ...
  const [showDietGenForm, setShowDietGenForm] = useState(false);
  ...
  const openDietGenForm = async () => {
    try {
      const profile = await api.getUserProfile();
      if (profile.weight) setDietTargetWeight(profile.weight.toString());
    } catch {}
    setDietGoal('maintain');
    setShowDietGenForm(true);
  };

  const handleGenerateDietPlan = async () => {
    setShowDietGenForm(false);
    setIsGeneratingDiet(true);
    try {
      const tw = parseFloat(dietTargetWeight);
      const plan = await api.generateDietPlan({
        targetWeight: !isNaN(tw) && tw > 0 ? tw : undefined,
        dietGoal: dietGoal || undefined,
      });
      trigger('success');
  ...
  const handleTabChange = (tab: TabType) => {
    trigger('selection');
    setActiveTab(tab);
    const positions = { diary: 0, nutrients: 1, trends: 2, goals: 3, diet: 4 };
  ...
  const renderDietPlanDetail = (plan: any) => (
    <FadeIn delay={100}>
      <TouchableOpacity
      ...
      >
        ...
        <Text style={styles.dietBackText}>All Plans</Text>
      </TouchableOpacity>

      <GlassCard style={styles.dietDetailHeader}>
        <Text style={styles.dietDetailName}>{plan.name}</Text>
        {plan.bmr && plan.tdee && (
          <Text style={styles.dietDetailSubtext}>
            BMR: {Math.round(plan.bmr)} | TDEE: {Math.round(plan.tdee)}
          </Text>
        )}
      </GlassCard>

      <View style={styles.dietStatRow}>
        {[
          { label: 'Calories', value: Math.round(plan.targets?.calories || 0), unit: 'kcal', color: COLORS.primary },
          { label: 'Protein', value: Math.round(plan.targets?.protein || 0), unit: 'g', color: COLORS.primary },
          { label: 'Carbs', value: Math.round(plan.targets?.carbs || 0), unit: 'g', color: COLORS.info },
          { label: 'Fat', value: Math.round(plan.targets?.fat || 0), unit: 'g', color: COLORS.warning },
        ].map((stat, index) => (
          <AnimatedCard key={stat.label} style={styles.dietStatCard} delay={150 + index * 50}>
            <Text style={[styles.dietStatValue, { color: stat.color }]}>{stat.value}</Text>
            <Text style={styles.dietStatUnit}>{stat.unit}</Text>
            <Text style={styles.dietStatLabel}>{stat.label}</Text>
          </AnimatedCard>
        ))}
      </View>

      {(plan.meals || []).map((meal: any, mealIndex: number) => (
        <AnimatedListItem key={mealIndex} index={mealIndex} enterFrom="right">
          <GlassCard style={styles.dietMealCard}>
            <View style={styles.dietMealHeader}>
              <View style={styles.dietMealTitleRow}>
                <Ionicons
                  name={
                    meal.mealType === 'breakfast' ? 'sunny-outline' :
                    meal.mealType === 'lunch' ? 'partly-sunny-outline' :
              ...
              <Text style={styles.dietMealCals}>
                {Math.round(meal.totals?.calories || 0)} kcal
              </Text>
            </View>
            {(meal.items || []).map((item: any, itemIndex: number) => (
              <View key={itemIndex} style={styles.dietFoodItem}>
                <View style={{ flex: 1 }}>
                  <Text style={styles.dietFoodName}>{item.food?.name || 'Unknown'}</Text>
                  <Text style={styles.dietFoodServing}>
                    {item.servings} x {item.food?.servingSize}{item.food?.servingUnit ? ` ${item.food.servingUnit}` : ''}
                  </Text>
                </View>
                <View style={{ alignItems: 'flex-end' }}>
                  <Text style={styles.dietFoodCals}>{Math.round(item.calories || 0)} kcal</Text>
                  <View style={styles.dietFoodMacros}>
                    <Text style={[styles.dietFoodMacro, { color: COLORS.primary }]}>P{Math.round(item.protein || 0)}</Text>
                    <Text style={[styles.dietFoodMacro, { color: COLORS.info }]}>C{Math.round(item.carbs || 0)}</Text>
                    <Text style={[styles.dietFoodMacro, { color: COLORS.warning }]}>F{Math.round(item.fat || 0)}</Text>
                  </View>
                </View>
              </View>
            ))}
            <View style={styles.dietMealTotals}>
              <Text style={styles.dietMealTotalsText}>
                P: {Math.round(meal.totals?.protein || 0)}g  |  C: {Math.round(meal.totals?.carbs || 0)}g  |  F: {Math.round(meal.totals?.fat || 0)}g
              </Text>
            </View>
          </GlassCard>
        </AnimatedListItem>
  ...
  const renderDietTab = () => (
    <>
      {showDietDetail && selectedDietPlan ? (
        renderDietPlanDetail(selectedDietPlan)
      ) : (
        <>
          <SlideIn direction="bottom" delay={100}>
            <TouchableOpacity
              style={[styles.generateButton, isGeneratingDiet && styles.generateButtonDisabled]}
              onPress={openDietGenForm}
            ...
            >
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={styles.generateButtonSubtitle}>
                  {isGeneratingDiet ? 'Creating your personalized plan' : 'Set your goal and target weight'}
                </Text>
              </View>
              {!isGeneratingDiet && (
          ...
          {dietPlans.length > 0 && (
            <FadeIn delay={200}>
              <Text style={styles.dietSectionTitle}>Your Diet Plans</Text>
            </FadeIn>
          )}

          {dietPlans.map((plan, index) => (
            <AnimatedListItem key={plan.id || index} index={index} enterFrom="right">
              <TouchableOpacity
                activeOpacity={0.8}
                onPress={() => {
                  trigger('light');
              ...
              >
                <GlassCard style={styles.dietPlanCard}>
                  <View style={styles.dietPlanInfo}>
                    <View style={{ flex: 1 }}>
                      <Text style={styles.dietPlanName}>{plan.name}</Text>
                      <Text style={styles.dietPlanDate}>
                        {plan.createdAt ? new Date(plan.createdAt).toLocaleDateString('en-US', {
                          month: 'short', day: 'numeric', year: 'numeric'
                        }) : ''}
                      </Text>
                      <Text style={styles.dietPlanMacros}>
                        {Math.round(plan.targets?.calories || 0)} kcal  |  P: {Math.round(plan.targets?.protein || 0)}g  C: {Math.round(plan.targets?.carbs || 0)}g  F: {Math.round(plan.targets?.fat || 0)}g
                      </Text>
                    </View>
                    <View style={styles.dietPlanActions}>
                      <TouchableOpacity
                        style={styles.dietDeleteBtn}
                        onPress={() => handleDeleteDietPlan(plan.id)}
          ...
          {dietPlans.length === 0 && !isGeneratingDiet && (
            <FadeIn delay={300}>
              <View style={styles.dietEmptyState}>
                <Ionicons name="restaurant-outline" size={48} color={COLORS.textTertiary} />
                <Text style={styles.dietEmptyTitle}>No Diet Plans Yet</Text>
                <Text style={styles.dietEmptySubtitle}>
                  Generate your first personalized diet plan above
                </Text>
              </View>
            </FadeIn>
          )}
  ...
  return (
    <View style={styles.container}>
        ...
        <FadeIn delay={50}>
          ...
          >
            {[
              ...
              { id: 'diet', label: 'Diet Plan', icon: 'restaurant-outline' },
            ].map((tab) => (
              <TouchableOpacity
                key={tab.id}
      ...
      <Modal
        visible={showDietGenForm}
        animationType="slide"
        presentationStyle="pageSheet"
        onRequestClose={() => setShowDietGenForm(false)}
      >
        <View style={styles.modalContainer}>
          <View style={styles.modalHeader}>
            <Text style={styles.modalTitle}>Generate Diet Plan</Text>
            <AnimatedButton
              variant="ghost"
              size="small"
              onPress={() => setShowDietGenForm(false)}
              title="Cancel"
              textStyle={{ color: COLORS.primary }}
            />
          </View>
          ...
          <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 16 }} keyboardShouldPersistTaps="handled">
            <GlassCard style={{ padding: 14, marginBottom: 16 }}>
              <Text style={[styles.sectionTitle, { marginBottom: 4 }]}>Target Weight</Text>
              <Text style={{ fontSize: 12, color: COLORS.textSecondary, marginBottom: 12 }}>
                Your desired weight (pre-filled with current)
              </Text>
              <View style={styles.weightInputContainer}>
                <TextInput
                  style={styles.weightInput}
                  ...
                  onChangeText={setDietTargetWeight}
                />
                <Text style={styles.weightUnit}>kg</Text>
              </View>
            </GlassCard>
            ...
            <GlassCard style={{ padding: 14, marginBottom: 16 }}>
              ...
              {[
                ...
                <TouchableOpacity
                  key={option.id}
                  style={[
                    styles.dietGoalOption,
                    dietGoal === option.id && styles.dietGoalOptionActive,
                  ]}
                  onPress={() => { trigger('selection'); setDietGoal(option.id); }}
                >
                  <Ionicons
                    name={option.icon as any}
                    size={22}
                    color={dietGoal === option.id ? '#fff' : COLORS.textSecondary}
                  />
                  <View style={{ flex: 1, marginLeft: 12 }}>
                    <Text style={[
                      styles.dietGoalLabel,
                      dietGoal === option.id && { color: '#fff' },
                    ]}>
                      {option.label}
                    </Text>
                    <Text style={[
                      styles.dietGoalDesc,
                      dietGoal === option.id && { color: 'rgba(255,255,255,0.7)' },
                    ]}>
                      {option.desc}
                    </Text>
                  </View>
