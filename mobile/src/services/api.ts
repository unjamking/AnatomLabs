...
class ApiService {
  ...
  async createCustomPlan(data: { name: string; goal: string; daysPerWeek: number; workouts: any[] }): Promise<WorkoutPlan> {
    const response = await this.api.post('/workouts/plans/custom', data);
    return response.data.plan;
  }

  async updateWorkoutPlan(id: string, data: { name?: string; goal?: string; daysPerWeek?: number; workouts?: any[] }): Promise<WorkoutPlan> {
    const response = await this.api.put(`/workouts/plans/${id}`, data);
    return response.data.plan;
  }

  async deleteWorkoutPlan(id: string): Promise<void> {
    await this.api.delete(`/workouts/plans/${id}`);
  }
  ...
  async removePushToken(): Promise<void> {
    await this.api.delete('/notifications/push-token');
  }
  ...
  async updateUserProfile(userId: string, data: { name?: string; age?: number | null; weight?: number | null; height?: number | null }): Promise<any> {
    const response = await this.api.put(`/users/${userId}`, data);
    return response.data;
  }

  async changePassword(currentPassword: string, newPassword: string): Promise<{ message: string }> {
    const response = await this.api.put('/users/me/change-password', { currentPassword, newPassword });
    return response.data;
  }
  ...
  async getCustomExercises(): Promise<any[]> {
    const response = await this.api.get('/custom-exercises');
    return response.data;
  }

  async createCustomExercise(data: { name: string; muscleGroup: string; equipment?: string; instructions?: string; notes?: string }): Promise<any> {
    const response = await this.api.post('/custom-exercises', data);
    return response.data;
  }

  async updateCustomExercise(id: string, data: any): Promise<any> {
    const response = await this.api.put(`/custom-exercises/${id}`, data);
    return response.data;
  }

  async deleteCustomExercise(id: string): Promise<void> {
    await this.api.delete(`/custom-exercises/${id}`);
  }

  async generateDietPlan(params?: { targetWeight?: number; dietGoal?: string }): Promise<any> {
    const response = await this.api.post('/diet-plans/generate', params || {});
    return response.data;
  }

  async getDietPlans(): Promise<any[]> {
    const response = await this.api.get('/diet-plans');
    return response.data;
  }

  async deleteDietPlan(id: string): Promise<void> {
    await this.api.delete(`/diet-plans/${id}`);
  }
