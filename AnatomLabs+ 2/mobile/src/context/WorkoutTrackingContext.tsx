...
export function WorkoutTrackingProvider({ children }: { children: ReactNode }) {
  ...
  const completeWorkout = useCallback(async (notes?: string): Promise<CompletedWorkout> => {
    ...
    activeWorkout.exercises.forEach(exercise => {
      ...
      const maxReps = Math.max(...exercise.sets.filter(s => !s.isWarmup).map(s => s.reps));
      const maxVolume = Math.max(...exercise.sets.filter(s => !s.isWarmup).map(s => s.weight * s.reps));

      const existingHistory = exerciseHistoryMap.get(exercise.exerciseId);
      if (existingHistory) {
        if (maxWeight > existingHistory.personalRecords.maxWeight) {
          newPRs.push({
            id: generateId(),
